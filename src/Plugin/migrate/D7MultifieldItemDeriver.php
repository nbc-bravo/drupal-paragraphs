...
/**
 * Deriver for multifields.
 */
class D7MultifieldItemDeriver extends DeriverBase implements ContainerDeriverInterface {
  ...
  /**
   * D7MultifieldItemDeriver constructor.
   *
  ...
  public function getDerivativeDefinitions($base_plugin_definition) {
    $types = static::getSourcePlugin('d7_multifield_type');
    ...
    try {
      foreach ($types as $row) {
        ...
        $multifield_bundle = $row->getSourceProperty('field_name');
        $p_bundle = $row->getSourceProperty('bundle');
        $values['label'] = t('@label (@type)', [
          '@label' => $values['label'],
          '@type' => $row->getSourceProperty('name'),
        ]);
        $values['source']['field_name'] = $multifield_bundle;
        $values['destination']['default_bundle'] = $p_bundle;

        /** @var \Drupal\migrate\Plugin\Migration $migration */
        $migration = \Drupal::service('plugin.manager.migration')
          ->createStubMigration($values);
        if (isset($fields[$multifield_bundle])) {
          $migration->setProcessOfProperty('parent_id', 'entity_id');
          $migration->setProcessOfProperty('parent_type', 'entity_type');
          $migration->setProcessOfProperty('parent_field_name', 'field_name');

          foreach ($fields[$multifield_bundle] as $field_name => $info) {
